...
export const Shop_3 = ({ onBackProducts, title }: TProps) => {
  ...
  const withOverlay = (Object.keys(PRODUCTS) as TTitleOptions[]).filter(
    key => PRODUCTS[key].title.includes('с наплавкой')
  );
  const withoutOverlay = (Object.keys(PRODUCTS) as TTitleOptions[]).filter(
    key => PRODUCTS[key].title.includes('без наплавки')
  );
